  ...
  useEffect(() => {
    ...
    const fetchDataList = async () => {
      try {
        ...
        console.error("자료실 목록 가져오기 오류:", error);
        setLoading(false);
      }
    };
  ...
  return (
    <>
      ...
      <BreadcrumbsSection className="breadcrumbs">
        <Container>
          <BreadcrumbsTitle>자료실</BreadcrumbsTitle>
          <BreadcrumbsText>
                  ...
                  <TableResponsive>
                    <Table>
                      ...
                      <TableBody>
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                {cate}
                                <Link
                                  to={`/Data/Detail/${notice.no}?Page=${page}`}
                                >
                                  {notice.subject}
                                </Link>
                                {isNew && (
                              ...
                              <td
                                ...
                                dangerouslySetInnerHTML={{
                                  __html: notice.fileDownListLink,
                                }}
                              />
                        ...
                        {data.datas.map((item) => {
                          // new 이미지 처리
                          const gap = new Date() - new Date(item.reg_date);
                          const userMinutes = 7 * 1440 * 60 * 1000; // 7일을 밀리초로 변환
                          const isNew = gap < userMinutes;

                          const cate = item.cate ? `[${item.cateName}]` : "";
                          ...
                          return (
                            <tr key={`data-${item.no}`}>
                              ...
                              >
                                {cate}
                                <Link
                                  to={`/Data/Detail/${item.no}?Page=${page}`}
                                >
                                  {item.subject}
                                </Link>
                                {isNew && (
                              ...
                              <td className="mb_none">
                                {Number(item.read_cnt).toLocaleString()}
                              </td>
                            </tr>
                          );
                        })}

                        {data.gongs.length === 0 && data.datas.length === 0 && (
                          <tr>
                ...
                <div className="text-center">
                  <Pagination>
                    ...
                    {page > 1 && (
                      <PageItem>
                        <PageLink
                          to={`/Data/Index?Page=1&SearchField=${searchField}&SearchQuery=${searchQuery}`}
                        >
                          &laquo;
                        </PageLink>
                      </PageItem>
                    ...
                    {Array.from(
                      ...
                      <PageItem key={p}>
                        <PageLink
                          to={`/Data/Index?Page=${p}&SearchField=${searchField}&SearchQuery=${searchQuery}`}
                          active={p === page}
                        >
                          {p}
                        </PageLink>
                    ...
                    {page < totalPages && (
                      <PageItem>
                        <PageLink
                          to={`/Data/Index?Page=${totalPages}&SearchField=${searchField}&SearchQuery=${searchQuery}`}
                        >
                          &raquo;
                        </PageLink>
                      </PageItem>
