...
const QnaModify = () => {
    ...
    const checkLoginStatus = async () => {
      try {
        ...
        if (!response.data.isLoggedIn) {
          alert("로그인이 필요합니다.");
          navigate("/Member/Login?returnUrl=/Qna/Mod/" + id);
          return false;
        }
        return true;
      } catch (error) {
    ...
    const fetchQnaData = async () => {
      try {
        ...
        setFormData({
          ...
          subject: qna.subject,
          content: qna.content,
          oldFilename1: qna.filename1 || "",
          oldFilesize1: qna.filesize1 || 0,
        });

        if (qna.filename1) {
          setPreviousFile(qna.filename1);
        }

        setLoading(false);
      } catch (error) {
        console.error("Q&A 정보 가져오기 오류:", error);
        alert("Q&A 정보를 가져오는 중 오류가 발생했습니다.");
        navigate("/Qna");
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    submitData.append("oldFilename1", formData.oldFilename1);
    submitData.append("oldFilesize1", formData.oldFilesize1);
    ...
    try {
      const response = await axios.post(`/api/qna/mod/${id}`, submitData, {
        headers: {
      ...
      if (response.data.success) {
        ...
        navigate(
          `/Qna/Detail/${id}?Page=${page}&SearchField=${searchField}&SearchQuery=${searchQuery}`
        );
      } else {
        alert(response.data.message || "Q&A 글 수정에 실패했습니다.");
      }
    } catch (error) {
      console.error("Q&A 수정 오류:", error);
      alert("Q&A 글 수정 중 오류가 발생했습니다.");
    }
  };
