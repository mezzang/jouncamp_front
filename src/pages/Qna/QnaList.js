  ...
  useEffect(() => {
    ...
    const fetchQnaList = async () => {
      try {
        ...
        console.error("Q&A 목록 가져오기 오류:", error);
        setLoading(false);
      }
    };
                  ...
                  <TableResponsive>
                    <Table>
                      ...
                      <TableBody>
                        {qnas.map((qna) => {
                          // new 이미지 처리
                          const gap = new Date() - new Date(qna.reg_date);
                          ...
                          let replyStr = "";
                          if (qna.tdepth > 0) {
                            for (let i = 0; i <= qna.tdepth; i++) {
                              replyStr += "\u00A0\u00A0"; // Non-breaking space
                            }
                            replyStr += '<img src="/img/btn_reply.gif" /> ';
                          }
                          ...
                          return (
                            <tr key={qna.no}>
                              ...
                              >
                                <span
                                  dangerouslySetInnerHTML={{ __html: replyStr }}
                                />
                                <Link
                                  to={`/Qna/Detail/${qna.no}?Page=${page}&SearchField=${searchField}&SearchQuery=${searchQuery}`}
                                >
                                  {qna.subject}
                                </Link>
                                {isNew && (
                              ...
                              <td className="mb_none">
                                {Number(qna.read_cnt).toLocaleString()}
                              </td>
                            </tr>
                          );
                        })}
                ...
                <div className="text-center">
                  <Pagination>
                    ...
                    {page > 1 && (
                      <PageItem>
                        <PageLink
                          to={`/Qna/Index?Page=1&SearchField=${searchField}&SearchQuery=${searchQuery}`}
                        >
                          &laquo;
                        </PageLink>
                      </PageItem>
                    ...
                    {Array.from(
                      ...
                      <PageItem key={p}>
                        <PageLink
                          to={`/Qna/Index?Page=${p}&SearchField=${searchField}&SearchQuery=${searchQuery}`}
                          active={p === page}
                        >
                          {p}
                        </PageLink>
                    ...
                    {page < totalPages && (
                      <PageItem>
                        <PageLink
                          to={`/Qna/Index?Page=${totalPages}&SearchField=${searchField}&SearchQuery=${searchQuery}`}
                        >
                          &raquo;
                        </PageLink>
                      </PageItem>
